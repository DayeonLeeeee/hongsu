...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("  suma v2 서버 (vision + tool_use + 벡터 분류 + 관리 API)")
    print(f"  로컬: http://localhost:5000")
    print(f"  네트워크: http://{local_ip}:5000")
    print("=" * 60)
    print("  AI 파이프라인:")
    print("    POST /ocr              - 이미지 OCR (평문 표시용)")
    print("    POST /solve            - 문제 이미지 → 정답/풀이")
    print("    POST /refine           - 자연어 지시로 평문 수정")
    print("    POST /grade            - 채점 (특성 벡터 + H분류 + 피드백)")
    print("    POST /master_solution  - 마스터 해설 생성")
    print("    POST /measure_consistency - 재현 일관성 측정")
    print("  관리 API:")
    print("    GET  /admin/problems       - 문제 목록 (?unit=함수)")
    print("    POST /admin/problems       - 문제 추가")
    print("    PUT  /admin/problems/<id>  - 문제 수정")
    print("    POST /admin/problems/bulk  - 일괄 등록")
    print("  결과 저장:")
    print("    POST /results              - 채점 결과 저장")
    print("    GET  /results              - 결과 조회 (?problem_id=&student_id=)")
    print("    GET  /stats/error_distribution - 오답 유형 분포")
    print("  기타:")
